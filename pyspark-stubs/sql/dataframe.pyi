...
from typing import Any, Callable, Iterator, Literal, overload, Generic, TypeVar, Union
...
class _JoinContext(Generic[T, T2]):
    ...
    def select(self, *cols: Union[Column[Union[T, T2], OutOther], Column[Literal["lit"], OutLit]]) -> DataFrame[OutOther]: ...
...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin, Generic[T]):
    ...
    @overload
    def repartition(self, numPartitions: int, *cols: ColumnOrName[T, LiteralString]) -> DataFrame[T]: ...
    @overload
    def repartition(self, *cols: ColumnOrName[T, LiteralString]) -> DataFrame[T]: ...
    @overload
    def repartitionByRange(self, numPartitions: int, *cols: ColumnOrName[T, LiteralString]) -> DataFrame[T]: ...
    @overload
    def repartitionByRange(self, *cols: ColumnOrName[T,LiteralString]) -> DataFrame: ...
    ...
    @property
    def columns(self) -> Column[T, T]: ...
    def colRegex(self, colName: str) -> Column: ...
    def to(self, schema: StructType) -> DataFrame: ...
    def alias(self, alias: str) -> DataFrame[T]: ...
    def crossJoin(self, other: DataFrame) -> DataFrame: ...
    def join(self, other: DataFrame[T2], on: Column[Union[T,T2], LiteralString] | None = None, how: str | None = None) -> _JoinContext[T,T2]: ... 
    ...
    @overload
    def select(self, *cols: Union[Column[T, OutOther],             Column[Literal["lit"], OutLit]]) -> DataFrame[OutOther]: ...
    @overload
    def select(self, /, __cols: list[Column[InOther,OutOther]]) -> DataFrame[OutOther]: ...
    @overload
    ...
    def unionByName(self, other: DataFrame[T2], allowMissingColumns: bool = False) -> DataFrame[Union[T,T2]]: ...
    ...
    def dropDuplicatesWithinWatermark(self, subset: list[str] | None = None) -> DataFrame[T]: ...
    ...
    def withColumns(self, *colsMap: dict[T2, Column[T, Out]]) -> DataFrame[Union[T,T2]]: ...
    def withColumn(self, colName: T2, col: Column[T, Out]) -> "DataFrame[Union[T, T2]]": ...
    ...
    @overload
    def drop(self, cols: ColumnOrName) -> DataFrame[T]: ...
    @overload
    def drop(self, *cols: T) -> DataFrame[T]: ...
